...
export const M = {
  ...
  dice_micro:  {label:'Dice (micro)',unit:'',   hi:true, dp:3, def:'Same formula with all pixels of all scenes pooled together. Dominated by large plumes.', formula:'2·ΣTP / (2·ΣTP + ΣFP + ΣFN)'},
  ...
  precision:   {label:'Precision',   unit:'',   hi:true, dp:3, def:'Share of predicted plume pixels that are correct. Low precision means false alarms.', formula:'TP / (TP + FP)'},
  recall:      {label:'Recall',      unit:'',   hi:true, dp:3, def:'Share of true plume pixels that were found. Low recall means missed signal.', formula:'TP / (TP + FN)'},
  ...
  bg_fp_rate:  {label:'Background FP',unit:'',  hi:false,dp:5, def:'On scenes with no plume at all, the fraction of pixels wrongly predicted as plume. Lower is better.', formula:'FP / (all pixels)'},
  ...
  tp:{label:'TP',unit:'px',hi:true,dp:0,def:'True positives: pixels correctly predicted as plume.',formula:'|P ∩ G|'},
  fp:{label:'FP',unit:'px',hi:false,dp:0,def:'False positives: predicted plume where there is none.',formula:'|P \\ G|'},
  fn:{label:'FN',unit:'px',hi:false,dp:0,def:'False negatives: true plume the model missed.',formula:'|G \\ P|'},
