...
export async function render(mount) {
  ...
  dispersal plume. This page summarises which model was chosen, how well it works, and where it fails.</p>
  ...
  <div class="cards">
    ...
    ${card('Precision', fmtOr(sel.precision,'precision'), 'of predicted plume', 'precision')}
    ${card('Recall', fmtOr(sel.recall,'recall'), 'of true plume found', 'recall')}
    ${card('Boundary IoU', fmtOr(sel.boundary_iou,'boundary_iou'), 'edge placement', 'boundary_iou')}
  </div>
  <p class="small">Measured on <b>${int(sel.n_pos_scenes)}</b> test scenes that contain a plume, using
  ...
  <div class="meta">
    ...
    <div><div class="l">Scenes with a plume</div><div class="d">${int(totalPos)}</div></div>
    <div><div class="l">Plume-free scenes</div><div class="d">${int(totalNeg)}</div></div>
  ...
  <div class="three">
    <div class="panel"><h3 style="margin-top:0;color:var(--ok)">Strengths</h3><ul class="small" style="padding-left:18px;line-height:1.7">
      <li>Finds ${pct(sel.recall,0)} of true plume pixels.</li>
      <li>Few false alarms on plume-free scenes (${pct(sel.bg_fp_rate,2)} of pixels).</li>
    ...
    <div class="panel"><h3 style="margin-top:0;color:var(--warn)">Weaknesses</h3><ul class="small" style="padding-left:18px;line-height:1.7">
      <li>Precision is only ${pct(sel.precision,0)}: it over-calls plume area.</li>
      <li>Score is capped near 0.63 Dice regardless of architecture.</li>
      <li>Small, sparse plumes score far worse than large ones
